          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                { val: 'returned', label: '🔁 Ibalik', color: 'border-orange-300 bg-orange-50 text-orange-700' },
...
export default function ApplicationTracking() {
  ...
  const tabs = isAdmin
    ? ['Lahat', 'Nakabinbin', 'Sinusuri', 'Inaprubahan', 'Ibinabalik', 'Tinanggihan']
    : ['Lahat', 'Nakabinbin', 'Inaprubahan', 'Ibinabalik'];
  ...
  const filteredApplications = allApplications.filter(app => {
    ...
    if (activeTab === 'Sinusuri') return matchesSearch && app.status === 'for_review';
    if (activeTab === 'Inaprubahan') return matchesSearch && (app.status === 'approved' || app.status === 'completed');
    if (activeTab === 'Ibinabalik') return matchesSearch && app.status === 'returned';
  ...
  const stats = [
    ...
    { label: 'Para sa Paglilinaw', count: allApplications.filter(a => a.status === 'returned').length, icon: AlertCircle, color: 'orange' },
                    ...
                    <td className="px-10 py-6">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button
                          ...
                          className="p-2.5 rounded-xl border border-slate-100 text-slate-300 hover:text-blue-600 hover:border-blue-200 transition-all"
                          title="Tingnan"
                        >
                        ...
                        {isAdmin && (
                          <button
                            ...
                            className="p-2.5 rounded-xl border border-slate-100 text-slate-300 hover:text-emerald-600 hover:border-emerald-200 transition-all"
                            title="Suriin / Desisyon"
                          >
                        ...
                        {confirmDeleteId === app.id ? (
                          <div className="flex items-center gap-1 animate-in fade-in zoom-in duration-200">
                            <button
                              ...
                              className="px-3 py-2 bg-red-600 text-white rounded-lg text-[9px] font-black uppercase tracking-widest hover:bg-red-700 transition-all flex items-center gap-1"
                            >
                              {deletingId === app.id ? <Loader2 className="w-3 h-3 animate-spin" /> : 'Kumpirmahin'}
                            </button>
                            <button onClick={() => setConfirmDeleteId(null)} className="p-2 bg-slate-100 rounded-lg hover:bg-slate-200 transition-all">
                          ...
                          <button
                            ...
                            className="p-2.5 rounded-xl border border-slate-100 text-slate-300 hover:text-red-600 hover:border-red-200 transition-all"
                            title="Burahin"
                          >
